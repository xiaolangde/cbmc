    ...
    if(symex_config.havoc_undefined_functions)
    {
      ...
      for(const auto &arg : cleaned_arguments)
      {
        if(
          ...
          !to_pointer_type(arg.type()).subtype().get_bool(ID_C_constant) &&
          to_pointer_type(arg.type()).subtype().id() != ID_code)
        {
          exprt object =
            dereference_exprt(arg, to_pointer_type(arg.type()).subtype());
